...
namespace EnvironmentAssessment.Common.Shell
{
	internal class WindowChromeWorker : DependencyObject
	{
		...
		private void _FixupFrameworkIssues()
		{
			...
			Rect rect = DpiHelper.DeviceRectToLogical(new Rect((double)windowRect.Left, (double)windowRect.Top, (double)windowRect.Width, (double)windowRect.Height));
			Rect rect2 = DpiHelper.DeviceRectToLogical(new Rect((double)rECT.Left, (double)rECT.Top, (double)rECT.Width, (double)rECT.Height));
		...
		private void _FixupWindows7Issues()
		{
			...
			if (Utility.IsOSWindows7OrNewer && NativeMethods.DwmIsCompositionEnabled())
			{
				...
				try
				{
					flag = NativeMethods.DwmGetCompositionTimingInfo(this._hwnd).HasValue;
				}
				catch (Exception)
				{
				}
		...
		{
			foreach (KeyValuePair<WM, MessageHandler> current in this._messageTable)
			{
				if (current.Key == (WM)msg)
				{
					return current.Value((WM)msg, wParam, lParam, out handled);
				}
			}
			return IntPtr.Zero;
		}
