...
namespace EnvironmentAssessment.Controls
{
    public class MessageBoxEx
    {
        ...
        private static void CenterWindow(IntPtr hChildWnd)
        {
            ...
            RECT locChild = new RECT(0, 0, 0, 0);
            bool success = GetWindowRect(hChildWnd, ref locChild);
            int childWidth = locChild.Right - locChild.Left;
            int childHeight = locChild.Bottom - locChild.Top;

            
            RECT locParent = new RECT(0, 0, 0, 0);
            success = GetWindowRect(_owner, ref locParent);
            int parentWidth = locParent.Right - locParent.Left;
            int parentHeight = locParent.Bottom - locParent.Top;

            int posX = locParent.Right - (parentWidth /2) - (childWidth /2);
            int posY = locParent.Bottom - (parentHeight / 2) - (childHeight /2);

            int result = MoveWindow(hChildWnd, posX, posY, childWidth, childHeight, false);
